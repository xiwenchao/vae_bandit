...
class MultiVAE():
    def __init__(self, p_dims, r_dims, q_dims=None, lam=0.01, lr=2e-4, random_seed=98765):
        self.p_dims = p_dims
    ...
    def build_graph(self):
        ...
        reg_var = apply_regularization(reg, self.weights_q + self.weights_p + self.weights_r)
...
def evaluate(input_generator, train_op_var, vae, sampled_z, sess, expected_reward, epsilon):
    score1, score2 = 0.0, 0
...
def run(log_file, train_op_var, vae, sampled_z, sess, expected_reward, epsilon):
    with open(log_file, 'r', buffering=1024 * 1024 * 512) as inf:
        return evaluate(inf, train_op_var, vae, sampled_z, sess, expected_reward, epsilon)
...
if __name__ == "__main__":
    ...
    with tf.Session() as sess:
        ...
        starttime = time.time()
        run(log_file, train_op_var, vae, sampled_z, sess, expected_reward, epsilon)
        endtime = time.time()
        ...
        plt.title("batch_network_reward")  # 图标题

        plt.show()  # 显示图

        plt.savefig("batch_network_reward.jpg")  # 保存
